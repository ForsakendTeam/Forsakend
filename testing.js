(function() {
  ...
  var activeDotSnake = function(x, y, part){
    console.log("Snake part" + part);
    console.log("Score " + score);
    if(part > 0 && part < snake_textures.length && score >= part){
      ctx.drawImage(snake_textures[part-1], x * scale, y * scale, scale, scale);
    }
    else{
      activeDot(x,y);
    }
  ...
  var addFood = function() {
    food.x = Math.floor(Math.random() * ((canvas.width / scale) - 1));
    food.y = Math.floor(Math.random() * ((canvas.height / scale) - 1));
  ...
  var mainLoop = function() {
    ...
    if (wall == 1) {
      // On
      if (snake[0].x < 0 || snake[0].x == canvas.width / scale || snake[0].y < 0 || snake[0].y == canvas.height / scale) {
        showScreen(3);
        return;
      }
    } else {
      ...
      for (var i = 0, x = snake.length; i < x; i++) {
        if (snake[i].x < 0) {
          snake[i].x = snake[i].x + (canvas.width / scale);
        }
        if (snake[i].x == canvas.width / scale) {
          snake[i].x = snake[i].x - (canvas.width / scale);
        }
        if (snake[i].y < 0) {
          snake[i].y = snake[i].y + (canvas.height / scale);
        }
        if (snake[i].y == canvas.height / scale) {
          snake[i].y = snake[i].y - (canvas.height / scale);
        }
      }
    }
    ...
    for (var i = 0; i < snake.length; i++) {
      activeDotSnake(snake[i].x, snake[i].y, i);
    }
  ...
  window.onload = function() {
    ...
    screen_howtoplay = document.getElementById("howtoplay");  
